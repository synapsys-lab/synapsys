...
def _build_figure():
    ...
    ax = axes["force"]
    ax.set_title(
        "Control force  F(t)  [Neural-LQR]", color="#e2e8f0", fontsize=10, pad=6
    )
    ...
    ax.axhline(
        -U_LIMIT, color=RED, ls=":", lw=0.8, alpha=0.4, label=f"±{U_LIMIT} N sat."
    )
    ...
    ax = axes["phase"]
    ax.set_title(
        "Phase portrait  (x₁, x₂) trajectory → equilibrium",
        color="#e2e8f0",
        fontsize=10,
        pad=6,
    )
    ...
    (dot_ph,) = ax.plot(
        [], [], "o", color=ORANGE, ms=6, label="current state", zorder=5
    )
    ax.legend(
        fontsize=8,
        loc="upper left",
