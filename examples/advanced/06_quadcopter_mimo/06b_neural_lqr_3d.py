...
def _show_config_dialog() -> SimConfig | None:
    ...
    def _slider_row(
        ...
        tk.Label(
            row, text=label, bg=PANEL, fg=FG, font=FONT, width=22, anchor="w"
        ).pack(side="left")
        val_lbl = tk.Label(
            row, text=fmt.format(var.get()), bg=PANEL, fg=ACCENT, font=FONT, width=7
        )
    ...
    tk.Label(
        root,
        text="Quadcopter MIMO — Simulation Config",
        bg=BG,
        fg=FG,
    ...
    tk.Label(
        root,
        text="Configure parameters and click  Run Simulation",
        bg=BG,
        fg=BORDER,
        font=FONT_S,
    ).pack(pady=(0, 8))
    ...
    _slider_row(
        sec_time, "Takeoff hover phase  (s)", v_thover, 1.0, 15.0, 0.5, "{:.1f} s"
    )
    ...
    _slider_row(
        sec_f8, "Angular speed  ω  (rad/s)", v_f8omega, 0.1, 0.8, 0.05, "{:.2f} r/s"
    )
    ...
    _slider_row(
        sec_ci, "Angular speed  ω  (rad/s)", v_comega, 0.1, 0.8, 0.05, "{:.2f} r/s"
    )
...
def _run_fast_sim(
    ...
    for step in range(n_steps):
        ...
        if step % 500 == 0:
            print(
                f"  sim {100 * step / n_steps:.0f}%  t={t:.1f}s", end="\r", flush=True
            )
...
def _save_matplotlib_gif(
    ...
    ani = FuncAnimation(
        fig, _frame, frames=len(idxs), interval=int(1000 / fps), blit=False
    )
    ani.save(
        str(path), writer=PillowWriter(fps=fps), dpi=80, progress_callback=_progress
    )
    plt.close(fig)
    size_kb = path.stat().st_size / 1024
...
def _build_mpl_figure(cfg: SimConfig) -> tuple:
    ...
    ax_ang.set_title(
        "Euler angles  phi, theta, psi", color="#e2e8f0", fontsize=10, pad=5
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--fps", type=int, default=15, help="PyVista GIF fps (default 15)"
    )
    parser.add_argument(
        "--mpl-fps", type=int, default=0, help="matplotlib GIF fps (default: fps//2)"
    )
    parser.add_argument(
        "--out",
        type=str,
    ...
    sys_d = c2d(sys_c, DT)
    print(
        f"Plant: {sys_c.n_states} states · {sys_c.n_inputs} inputs "
