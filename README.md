...
| v0.2.6 | Released | `info["failed"]` hook, `linearised` CartPole, `PlantAgent`+`SimulatorBase`, `CartPole2DView`, camera presets, trajectory trail, 541 tests |
| v0.2.7 | Released | `Light` palette, `mpl_theme("light")`, `save=` on all SimViews, `_trail_point()` hook, integrator benchmark, simulator docs, viz API reference update |
