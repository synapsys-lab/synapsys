...
class TestFeedback:
    ...
    def test_ss_siso_unity_feedback_pole(self):
        G = StateSpace([[-1]], [[1]], [[1]], [[0]])
        T = feedback(G)
        np.testing.assert_allclose(np.sort(np.real(T.poles())), [-2.0], atol=1e-8)
    ...
    def test_ss_mimo_unity_feedback_dimensions(self):
        ...
        G = StateSpace(np.diag([-1.0, -2.0]), np.eye(2), np.eye(2), np.zeros((2, 2)))
        T = feedback(G)
        assert isinstance(T, StateSpace)
        assert T.n_inputs == 2
    ...
    def test_ss_siso_with_static_tf_sensor_no_phantom_state(self):
        ...
        G = StateSpace([[-1.0]], [[1.0]], [[1.0]], [[0.0]])
        H = TransferFunction([2.0], [1.0])  # static gain 2
        T = feedback(G, H)
    ...
    def test_feedback_dt_mismatch_raises(self):
        G = StateSpace([[-1.0]], [[1.0]], [[1.0]], [[0.0]])
        H = StateSpace([[-2.0]], [[1.0]], [[1.0]], [[0.0]], dt=0.1)
