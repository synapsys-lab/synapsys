...
K_lqr, _ = lqr(eqs.A, eqs.B,
               np.diag([1.0, 10.0, 0.5, 1.0]),
               np.array([[1.0]]))
...
for k in range(N):
    ...
    x1[k]=float(y[0]); x2[k]=float(y[1])
    v1[k]=float(y[2]); v2[k]=float(y[3]); u[k]=u_k
...
def _legend(ax, **kw):
    leg = ax.legend(fontsize=FS_LEGEND, facecolor=BG_AX,
                    edgecolor=COL_GRID, labelcolor=COL_TXT, **kw)
    return leg
...
fig.suptitle(
    ...
    color=COL_TXT, fontsize=FS_SUPTITLE, fontweight="bold", y=0.97,
)
...
# ── Position ──
_style_ax(ax_p, "Position tracking — x₁(t) and x₂(t) → setpoint", "Position (m)", "Time (s)")
ax_p.axhline(X2_REF, color=GREEN, ls="--", lw=1.4, alpha=0.7, label=f"setpoint  r = {X2_REF} m")
ax_p.axhspan(X2_REF*0.98, X2_REF*1.02, alpha=0.07, color=GREEN)
ax_p.axvline(t_settle, color=ORANGE, ls="--", lw=1.4, alpha=0.85,
...
ax_ph.set_title("Phase portrait  (x₁, x₂) — trajectory from rest to equilibrium",
                color=COL_TXT, fontsize=FS_TITLE, pad=7, fontweight="semibold")
...
ax_ph.plot(x1[-1], x2[-1], "s", ms=9,  color=ORANGE, zorder=5, label=f"final state ({x1[-1]:.2f}, {x2[-1]:.2f})")
ax_ph.axhline(X2_REF, color=GREEN, ls="--", lw=1.2, alpha=0.55, label=f"x₂ = {X2_REF}  (setpoint)")
...
_style_ax(ax2_p,  "Position tracking — x₁(t) and x₂(t) → setpoint",
          "Position (m)", "Time (s)")
...
ax2_ph.set_title("Phase portrait  (x₁, x₂)",
                 color=COL_TXT, fontsize=FS_TITLE, pad=7, fontweight="semibold")
...
ax2_p.axhline(X2_REF,       color=GREEN, ls="--", lw=1.4, alpha=0.6,
              label=f"setpoint  r = {X2_REF} m")
...
ax2_ph.axhline(X2_REF,       color=GREEN, ls="--", lw=1.2, alpha=0.5,
               label=f"x₂ = {X2_REF}")

# Pre-set fixed axis limits (avoids rescaling per-frame)
T_MAX = t[-1]
ax2_p.set_xlim(0, T_MAX);  ax2_p.set_ylim(min(x1.min(), x2.min()) - 0.15, max(x1.max(), x2.max()) + 0.25)
ax2_v.set_xlim(0, T_MAX);  ax2_v.set_ylim(min(v1.min(), v2.min()) - 0.1,  max(v1.max(), v2.max()) + 0.1)
ax2_f.set_xlim(0, T_MAX);  ax2_f.set_ylim(u.min() - 0.5, u.max() + 0.5)
...
dot_ph,  = ax2_ph.plot([], [], "o", ms=10, color=ORANGE, zorder=6, label="current state")

# Time annotation
time_ann = ax2_p.annotate("t = 0.00 s", xy=(0.01, 0.94),
                           xycoords="axes fraction",
                           color=ORANGE, fontsize=FS_LABEL, fontweight="bold")
...
x1_e = x1[::SUB]; x2_e = x2[::SUB]; u_e = u[::SUB]
dt_e = DT * SUB
