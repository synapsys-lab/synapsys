...
from synapsys.api import c2d, feedback, step, tf
...
ax.axvline(
    ...
    lw=1.2,
    alpha=0.7,
    label=f"peak at t={t_peak:.2f}s ({overshoot_pct:.1f}% OS)",
)
ax.axvline(
    t_settle,
    color=GREEN,
    ...
    lw=1.2,
    alpha=0.7,
    label=f"settled at t={t_settle:.2f}s",
)
ax.fill_between(t, y_ss * 0.98, y_ss * 1.02, alpha=0.07, color=GREEN)
ax.set_title(
    r"Step Response — $G(s)=\frac{100}{s^2+10s+100}$  ($\zeta=0.5$, $\omega_n=10$)",
    fontsize=11,
)
...
ax2.axhline(
    10 / 11, color=GRAY, ls="--", lw=1.2, alpha=0.6, label=f"DC gain = {10 / 11:.4f}"
)
...
fig.suptitle(
    r"$G(s)=\dfrac{10}{s+1}$ — open loop vs closed loop (unity negative feedback)",
    fontsize=11,
)
fig.tight_layout()
...
ax.step(t_df, y_df, color=GREEN, lw=1.5, where="post", label="ZOH Ts=0.05 s (20 Hz)")
ax.step(
    t_ds,
    y_ds,
    color=RED,
    ...
    where="post",
    linestyle="--",
    label="ZOH Ts=0.5 s  (2 Hz)",
)
ax.set_title(
    r"Discretisation — $G(s)=\dfrac{1}{(s+1)^2}$ — continuous vs ZOH", fontsize=11
)
ax.set_xlabel("Time (s)")
ax.set_ylabel("y(t)")
ax.set_xlim(0, 5)
...
fig.suptitle(
    r"PID closed-loop — $G(s)=\dfrac{25}{s^2+5s+25}$,  Kp=3, Ki=1.5, Kd=0", fontsize=11
)
fig.tight_layout()
